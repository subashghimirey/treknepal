...
class TimsApplicationSerializer(serializers.ModelSerializer):
    ...
    trek_id = serializers.IntegerField(write_only=True, required=False)
    ...
    def update(self, instance, validated_data):
        trek_id = validated_data.pop('trek_id', None)
        if trek_id is not None:
            instance.trek_id = trek_id
        for attr, value in validated_data.items():
            setattr(instance, attr, value)
        instance.save()
        return instance
    ...
    def create(self, validated_data):
        ...
        validated_data['trek_id'] = trek_id
